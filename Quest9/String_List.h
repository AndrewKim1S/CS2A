...
public:
    ...
    String_List *insert_at_current(std::string s) {
        Node *x = new Node(s);
        Node *past = _prev_to_current->next;
        _prev_to_current->next = x;
        x->next = past;
    ...
    std::string to_string() const {
        std::string result = "# String_List - " + std::to_string(get_size())
        + " entries total. Starting at cursor:\n";
        Node *runner = _prev_to_current;
        size_t i;
        int count = 0;
        for(i = _size -1; i >= 0; i--){
            count ++;
            result += runner->next->data + "\n";
            runner = runner->next;
            if(count == 25 || i == 0){
                break;
            }
